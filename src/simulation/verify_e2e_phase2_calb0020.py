...
measured SoH trajectory of CALB cell 0020.
...
Writes:
    data/synthetic/verification/calb_0020_deg_params.yaml
    data/synthetic/verification/calb_0020_phase2_progress.csv
    data/synthetic/verification/calb_0020_phase2_log.txt
"""
...
PROGRESS_CSV = OUT_DIR / "calb_0020_phase2_progress.csv"
LOG_TXT = OUT_DIR / "calb_0020_phase2_log.txt"
DEG_YAML = OUT_DIR / "calb_0020_deg_params.yaml"

CELL = "0020"
C_RATE = 0.5           # from Longterm CSV column
N_CYCLES = 150
MAX_WALLTIME_S = 30 * 60   # hard cap: 30 minutes for the DE run (per task)
...
def load_measured_soh() -> tuple[np.ndarray, np.ndarray]:
    ...
    # Normalize so first measured cycle = 1.0 — CALB 0020 starts at ~0.64 in
    # canonical (already-aged); we're fitting the shape from that reference.
    soh_norm = soh / soh[0]
