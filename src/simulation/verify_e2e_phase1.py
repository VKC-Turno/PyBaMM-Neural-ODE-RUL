"""
Phase 1: Per-cell BOL parameter identification for EVE cell 0008.

Reuses the existing param_id modules (ocv_fit, dcir_hppc, gitt_ds,
sei_selfdisc) but restricts to a single cell, so results are per-cell
rather than cohort medians.

Writes:
    data/synthetic/verification/eve_0008_bol_params.yaml
    data/synthetic/verification/eve_0008_phase1_log.txt
"""
...
def phase1() -> dict:
    ...
    if pulse_df.empty:
        ...
        pulse_df = pd.DataFrame([vars(p) for p in pulses])
    if pulse_df.empty:
        raise RuntimeError("No resistance pulses could be extracted for cell 0008")
    ...
    try:
        ...
        if gitt.empty:
            log("  GITT metrics empty — cell 0008 has no usable GITT data")
    ...
    # Save
    out_yaml = OUT_DIR / "eve_0008_bol_params.yaml"
    with open(out_yaml, "w") as f:
        yaml.safe_dump(result, f, sort_keys=False)
    log(f"\nWrote BOL params -> {out_yaml}")

    (OUT_DIR / "eve_0008_phase1_log.txt").write_text("\n".join(log_lines))
    return result
