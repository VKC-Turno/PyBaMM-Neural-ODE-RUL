"""Path B configuration extended to K=100 — target 7/7 under 3 pp.

Path B (bigger net, longer training) at K=50 gave 5/7 under 3 pp. At K=100
we have 2× more training data per cell. If Path B reaches 7/7 at K=100,
the headline becomes '4× reduction vs pure-physics K=400 with 7/7 coverage'
— cleaner than K=50's 5/7 with 2 misses.
"""
...
print(f"=== Path B config extended to K={K} ===")
...
df.to_csv(OUT / "pathB_K100.csv", index=False)
import pickle
with open(OUT / "pathB_K100_trajectories.pkl", "wb") as f:
    pickle.dump(trajectories, f)

print(f"{'='*66}")
print(f"{'cell':>5}  {'PINN test':>10}  {'phys test':>10}  {'winner':>8}  {'<3pp?':>6}")
print(f"{'='*66}")
for r in results:
    ...
    print(f"{r['cell_id']:>5}  {r['rmse_pinn_test_pp']:>7.3f} pp  "
          f"{r['rmse_phys_test_pp']:>7.3f} pp  {w:>8}  {p:>6}")
...
print(f"PINN <3pp: {n_pass}/7   phys <3pp: {int((df.rmse_phys_test_pp<3.0).sum())}/7")
if n_pass == 7:
    print("\n*** 7/7 AT K=100 — 4x reduction vs pure-physics K=400 ***")
