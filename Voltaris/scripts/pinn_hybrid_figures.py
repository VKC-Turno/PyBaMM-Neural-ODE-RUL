"""Regenerate make-agnostic figures using the hybrid PINN checkpoint.

Loads:
  - Voltaris/outputs/sciml_hybrid/hybrid_K50.pt
  - Voltaris/outputs/sciml_hybrid/hybrid_vs_baseline_K50.csv

Writes:
  - Voltaris/outputs/sciml_hybrid/make_agnostic_{calb,rept,eve}_grid_hybrid.png
  - Voltaris/outputs/sciml_hybrid/make_agnostic_bar_hybrid.png

Only runs if the hybrid checkpoint exists and hybrid beat the baseline on CALB
(the visual "flat-then-drop" artifact was the main motivation).
"""
...
CKPT = OUT / "hybrid_K50.pt"
CSV  = OUT / "hybrid_vs_baseline_K50.csv"
assert CKPT.exists(), f"missing checkpoint {CKPT} — run pinn_hybrid_K50.py first"

# ── Load cohort + rebuild model ──
...
model = JointPINN_Hybrid(n_cells=len(cells), n_shared_features=n_shared,
                          embed_dim=8, hidden=128, n_layers=5,
                          feat_mean=mean_s, feat_std=std_s, p_init=0.5)
model.load_state_dict(torch.load(CKPT, map_location=DEVICE))
model.to(DEVICE).eval()
cfg = HybridConfig(K=K, n_norm_scale=n_norm_scale, p_init=0.5)

# ── Rebuild trajectory dict from checkpoint ──
...
def make_grid(name, trajs, ncols=3):
    ...
    fig.suptitle(f"{name} — Hybrid PINN vs pure physics (K=50)\n"
...
for name, trajs in per_make.items():
    ...
    outfile = OUT / f"make_agnostic_{name.lower()}_grid_hybrid.png"
...
ax.set_title("Make-agnostic K=50 (hybrid): held-out RMSE per cell")
xticks, xlabels = [], []
for m in ["CALB", "REPT", "EVE"]:
    d = df_h[df_h.make == m].sort_values("cell_id").reset_index(drop=True)
...
fig.tight_layout()
fig.savefig(OUT / "make_agnostic_bar_hybrid.png", dpi=150, bbox_inches="tight")
plt.close(fig)
print("Wrote make_agnostic_bar_hybrid.png")
