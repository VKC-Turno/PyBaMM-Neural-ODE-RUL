"""CALB_old clean-cells reaction-limited SEI sweep.

For each clean long-trajectory CALB_old cell, calibrates pure rxn-lim SEI
...
no sensor zeros):
    6, 7, 10, 14, 19, 20, 24, 25, 30
Cell 25 is NOT in the char workbook → use cohort-median CALB_old char.
...
# ─────────────────────────── config ───────────────────────────
CELLS = [6, 7, 10, 14, 19, 20, 24, 25, 30]
...
def load_char_for(cid: int):
    ...
    sub = cells[(cells.cohort == "CALB_old") & (cells.cell_id.astype(str) == str(cid))]
    if not sub.empty:
        return load_characterization(cohort="CALB_old", cell_id=str(cid)), "cell-specific"
    return load_characterization(cohort="CALB_old", aggregate=True), "cohort-median"
...
def main():
    ...
    print(f"=== CALB_old clean-cells rxn-lim sweep ({len(CELLS)} cells) ===\n")
    results = []
    t_top = time.time()
    ...
    df.to_csv(OUT_DIR / "CALB_clean_rxnlim_sweep_summary.csv", index=False)
    print(f"\nSummary CSV: CALB_clean_rxnlim_sweep_summary.csv")
    ...
    fig.suptitle("CALB_old clean cells — pure rxn-lim SEI vs measured", fontsize=13, y=1.005)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "CALB_clean_rxnlim_sweep_grid.png", dpi=110)
    plt.close(fig)
    print(f"\n3x3 grid: CALB_clean_rxnlim_sweep_grid.png")
