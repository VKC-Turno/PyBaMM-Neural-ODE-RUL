"""MFR_A clean-cells rxn-lim hold-out experiment.

For each clean long-trajectory MFR_A cell that already fits pure rxn-lim
...
Reuses helpers from mfr_a_clean_cells_rxnlim_sweep.py so the physics stack is
...
from mfr_a_clean_cells_rxnlim_sweep import (  # noqa: E402
...
CELLS = [1, 2, 3, 4, 5, 6, 7]  # 24 & 30 excluded (batch-transition artifacts)
K_VALUES = [50, 100, 200, 400, 800]

OUT_DIR = REPO_ROOT / "outputs/holdout_sweep"
...
def main():
    ...
    df.to_csv(OUT_DIR / "MFR_A_holdout_sweep_summary.csv", index=False)
    print(f"\nSummary CSV: {OUT_DIR / 'MFR_A_holdout_sweep_summary.csv'}")
    ...
    if not df.empty:
        ...
        ax.set_title("MFR_A rxn-lim hold-out: prediction error vs training length")
