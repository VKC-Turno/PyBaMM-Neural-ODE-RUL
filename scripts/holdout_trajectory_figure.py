...
held-out region shaded. Picks cell 07 as the representative "clean" case
(RMSE_test=0.29 pp at K=100, delta_end=+0.21 pp — best-in-cohort extrapolation).

Companion to holdout_rmse_vs_K.png. Output: outputs/holdout_sweep/fig2_trajectory_cell_07.png
"""
...
from mfr_a_clean_cells_rxnlim_sweep import (
    load_measured, load_char_for, run_long_sim, calibrate_rxnlim,
)
from mfr_a_clean_cells_holdout_sweep import train_slope_pp_per_100cy


CELL_ID = 7
K_TRAIN = 100
OUT = REPO_ROOT / "outputs/holdout_sweep"
...
def main():
    ...
    ax.set_ylabel("SoH [%]")
    ax.set_title(f"MFR_A cell {CELL_ID}: PyBaMM rxn-lim SEI extrapolation "
    ...
    fig.tight_layout()
    out_path = OUT / "fig2_trajectory_cell_07.png"
    fig.savefig(out_path, dpi=180)
    print(f"Figure: {out_path}")

    # Also emit vector for LaTeX
    fig.savefig(OUT / "fig2_trajectory_cell_07.pdf")
    print(f"PDF: {OUT / 'fig2_trajectory_cell_07.pdf'}")
