...
training). Applied to a deep-second-life supplier-A cell at $K=50$,
the model forecasts RUL to a second-life EoSL threshold of
$\text{SoH} = 0.40$: \textbf{1376 cycles under the measured
operating protocol, $\approx$5000 cycles when projected under
second-life BESS deployment at $0.25$\,C} via an empirical LFP C-rate
fade-rate scaling ($3.5\times$ slowdown, Fig.~1). Both projections
decouple the cycling budget from the extrapolation horizon.
...
\begin{figure}[h!]
  ...
  \caption*{\footnotesize Figure 1: Neural-model RUL forecast for a
    deep-second-life supplier-A cell under two operating protocols.
    From $K=50$ measured cycles (orange band), the green solid line
    is the as-measured-protocol extrapolation reaching EoSL at cycle
    1428 (RUL 1376 cy); the blue dashed line projects the same cell
    under second-life BESS deployment at $0.25$\,C, reaching EoSL at
    cycle $\sim$5000 (RUL 4941 cy). The neural-operator formulation
    retains the K=50 input budget and extends output to the full
    remaining-life horizon via PyBaMM-generated training trajectories.}
\end{figure}
